...
app.get("/filter",(req,res)=>{
  ...
  const filteredActivies = jokes.filter( (joke)=>joke.jokeType === type );
  res.json(filteredActivies);
})
...
//looping through the jokes array to find the joke with the id

const replacementJoke = {
  id : id,
  jokeText : req.body.jokeText || existingJoke.jokeText,
  jokeType : req.body.jokeType || existingJoke.jokeType,
  //if the jokeText or jokeType is not provided, it will use the existing jokeText or jokeType
};
...
app.delete("/jokes", (req,res)=>{
  ...
  if( userKey === masterKey ){
    ...
    res
      .status(404)
      .json({error: "Invalid key. No jokes were deleted."});
  }
})
